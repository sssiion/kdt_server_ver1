...
public interface CashTransactionRepository extends JpaRepository<CashTransaction, Long> {
    ...
    List<CashTransaction> findByAccountNumber(Long accountNumber);
    // 계좌번호로 거래 내역 조회

    List<CashTransaction> findByAccountNumberOrderByTransactionDateDesc(Long accountNumber);
    // 계좌번호로 거래 내역을 최신순으로 조회

    List<CashTransaction> findByAccountNumberOrderByTransactionDateDesc(Long accountNumber, Pageable pageable);
    // 계좌번호로 거래 내역을 최신순으로 조회 (페이지네이션)

    List<CashTransaction> findByTransactionType(CashTransaction.TransactionType transactionType);
    // 거래 유형별 조회

    List<CashTransaction> findByAccountNumberAndTransactionType(Long accountNumber, CashTransaction.TransactionType transactionType);
    // 계좌번호 및 거래 유형별 조회

    List<CashTransaction> findByTransactionDateBetween(LocalDateTime startDate, LocalDateTime endDate);
    // 거래 날짜 기간으로 조회

    List<CashTransaction> findByAccountNumberAndTransactionDateBetween(Long accountNumber, LocalDateTime startDate, LocalDateTime endDate);
    ...
    List<CashTransaction> findByAccountNumberAndMinAmount(@Param("accountNumber") Long accountNumber, @Param("minAmount") BigDecimal minAmount);
    ...
    BigDecimal getTotalAmountByTypeAndPeriod(@Param("accountNumber") Long accountNumber,
    ...
    CashTransaction findLatestTransactionByAccountNumber(@Param("accountNumber") Long accountNumber);
    // 특정 계좌의 최근 거래 1건
    ...
    List<CashTransaction> findByAccountNumberAndTransactionTypeOrderByTransactionDateDesc(@Param("accountNumber") Long accountNumber,
                                                                                          @Param("transactionType") CashTransaction.TransactionType transactionType,
                                                                                          Pageable pageable);
